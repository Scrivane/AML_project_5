...
We addressed client participation in federated learning with
 two different strategies. First, we introduced minibatch
  probing to identify clients likely to produce informative updates,
   combined with a probe + fairness hybrid to prevent weaker clients
    from being excluded and to balance efficiency with equity.
     Second, we proposed Diversity Score Sampling, which promotes
      clients contributing novel information by considering both 
      intra- and inter-client diversity, thereby reducing redundancy 
      in updates. Both these approaches improve convergence 
      while ensuring fair and diverse client participation.
