...
\subsection{FedAvg}
For federated learning we used FedAvg \cite{Paper10} .It consists of a server and multiple clients. The server initializes a global model and sends it to a subset of clients (in case 10\% of them ). Each client trains the model on its local data for a few steps and sends the updated model back to the server.
